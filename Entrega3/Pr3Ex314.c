...
int main(int argc, char *argv[]){
	...
	fscanf(file, "%i\n", &NVE);
	fscanf(file, "%i\n", &NAR);
        printf("Hi ha %d vertexs i %d arestes \n",NVE,NAR);
    ...
    unsigned i,j,l,QJ,npos=0,seguir=0, MAXARNO = valencia(larestes, NAR,NVE), arestespos[MAXARNO];
    printf("Valencia màxima %u\n",MAXARNO);
    int arrcorr[NAR];    
    cami.naresvisit=0;
    do{
		for(i=0;i<NAR;i++){
