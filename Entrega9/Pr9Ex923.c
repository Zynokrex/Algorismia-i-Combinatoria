...
int main(){
    ...
    //Imprimir llargada carrer
    char ident[12];
    node *nodescarrer;
    unsigned total, seguent;
    double dist_total=0;
    ...
    for(i=0; i<total-1; i++){
		seguent=buscapunt(nodescarrer[i+1].id, punts, nnodes);
		for(unsigned j=0; j<nodescarrer[i].narst; j++){
			if(nodescarrer[i].arestes[j].numnode==seguent){
				dist_total=dist_total+nodescarrer[i].arestes[j].llargada;
			}
		}
	}
	printf("Distància total del carrer amb id=%s: %lf\n", ident, dist_total);
