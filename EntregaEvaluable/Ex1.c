    ...
    for(i=0;i<num_conf && num_conf<MAXCONF;i++){
        for(j=0;j<NGERRES && num_conf<MAXCONF;j++){
            for(k=0;k<NGERRES && num_conf<MAXCONF;k++){
                ...
                for(l=0;l<NGERRES;l++) configuracions[num_conf].continguts[l]=configuracions[i].continguts[l];
                configuracions[num_conf].continguts[k]=configuracions[num_conf].continguts[k]+configuracions[num_conf].continguts[j];
                configuracions[num_conf].continguts[j]=0;
                if(configuracions[num_conf].continguts[k]>gerres[k].capacitat){
                    configuracions[num_conf].continguts[j]=configuracions[num_conf].continguts[k]-gerres[k].capacitat;
                    configuracions[num_conf].continguts[k]=gerres[k].capacitat;
                }
